...
namespace EntFrm.Framework.Utility
{
    public class StringHelper
    {
        ...
        public static string MaskString(string value, int startLen = 4, int endLen = 4, char specialChar = '*')
        {
            try
            {
    ...
    public static string Remove(string sourceString, string removedString)
        ...
        /// <returns></returns>
        public static string DelLastColon(string origin)
        {
            if (origin.IndexOf(";") == -1)
            {
                return origin;
        ...
        /// <param name="sourceString"></param>
        /// <returns></returns>
        public static string DelInvisibleChar(string sourceString)
        {
        ...
        public static string ConvertToString(byte[] btSource)
        {
            return BitConverter.ToString(btSource).Replace("-", string.Empty);
        }

        public static byte[] ConvertToBytes(string strSource)
        {
            byte[] returnBytes = new byte[strSource.Length / 2];
            for (int i = 0; i < returnBytes.Length; i++)
                returnBytes[i] = Convert.ToByte(strSource.Substring(i * 2, 2), 16);
            return returnBytes;
        }
    }
}
