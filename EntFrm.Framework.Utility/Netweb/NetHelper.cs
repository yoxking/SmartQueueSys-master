...
namespace EntFrm.Framework.Utility
{
    ...
    public class NetHelper
    {
        public static T HttpPost<T>(string uri, object data, SerializationType serializationType)
        {
            ...
            {
                t = (T)SerializeHelper.XmlDeserialize(typeof(T), responseText);
            }
            else if (serializationType == SerializationType.Json)
            {
                t = (T)SerializeHelper.JsonDeserialize(typeof(T), responseText);
            }
            return t;
        }
        ...
        public static string HttpPost(string uri, object data, SerializationType serializationType)
        {
            ...
            else
            {
                if (serializationType == SerializationType.Xml)
                {
                    dataStr = SerializeHelper.XmlSerialize(data);
                    object o = SerializeHelper.XmlDeserialize(data.GetType(), dataStr);
                }
                else if (serializationType == SerializationType.Json)
                {
                    dataStr = SerializeHelper.JsonSerialize(data);
                }
            }
