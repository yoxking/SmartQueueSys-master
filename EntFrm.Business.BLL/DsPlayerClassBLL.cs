...
{
    public class DsPlayerClassBLL
    {

        private string connStr;
        private string appCode;

        public  DsPlayerClassBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IDsPlayerClass infoDAL = DsPlayerClassFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(DsPlayerClass info)
        {
            try
            {
        ...
        public  bool UpdateRecord(DsPlayerClass info)
        {
            try
            {
                     IDsPlayerClass  infoDAL = DsPlayerClassFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  DsPlayerClass GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IDsPlayerClass infoDAL = DsPlayerClassFactory.Create(this.connStr, this.appCode);
                DsPlayerClassCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public DsPlayerClassCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IDsPlayerClass infoDAL = DsPlayerClassFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "DsPlayerClass";
        ...
        public DsPlayerClassCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
