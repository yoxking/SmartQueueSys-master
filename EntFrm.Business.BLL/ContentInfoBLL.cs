...
{
    public class ContentInfoBLL
    {

        private string connStr;
        private string appCode;

        public  ContentInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IContentInfo infoDAL = ContentInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" ContNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(ContentInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sContNo))
            {
                IContentInfo infoDAL = ContentInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(ContentInfo info)
        {
            try
            {
                     IContentInfo  infoDAL = ContentInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  ContentInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IContentInfo infoDAL = ContentInfoFactory.Create(this.connStr, this.appCode);
                ContentInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public ContentInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IContentInfo infoDAL = ContentInfoFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "ContentInfo";
        ...
        public ContentInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
