...
{
    public class TicketStyleBLL
    {

        private string connStr;
        private string appCode;

        public  TicketStyleBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               ITicketStyle infoDAL = TicketStyleFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" StyleNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(TicketStyle info)
        {
            try
            {
            if (!ValidateRepeat(info.sStyleNo))
            {
                ITicketStyle infoDAL = TicketStyleFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(TicketStyle info)
        {
            try
            {
                     ITicketStyle  infoDAL = TicketStyleFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  TicketStyle GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                ITicketStyle infoDAL = TicketStyleFactory.Create(this.connStr, this.appCode);
                TicketStyleCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public TicketStyleCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               ITicketStyle infoDAL = TicketStyleFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "TicketStyle";
        ...
        public TicketStyleCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
