...
{
    public class TicketFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  TicketFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               ITicketFlows infoDAL = TicketFlowsFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" TFlowNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(TicketFlows info)
        {
            try
            {
            if (!ValidateRepeat(info.sTFlowNo))
            {
                ITicketFlows infoDAL = TicketFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(TicketFlows info)
        {
            try
            {
                     ITicketFlows  infoDAL = TicketFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  TicketFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                ITicketFlows infoDAL = TicketFlowsFactory.Create(this.connStr, this.appCode);
                TicketFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public TicketFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               ITicketFlows infoDAL = TicketFlowsFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "TicketFlows";
        ...
        public TicketFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
