...
{
    public class CallerInfoBLL
    {

        private string connStr;
        private string appCode;

        public  CallerInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               ICallerInfo infoDAL = CallerInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" CallerNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(CallerInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sCallerNo))
            {
                ICallerInfo infoDAL = CallerInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(CallerInfo info)
        {
            try
            {
                     ICallerInfo  infoDAL = CallerInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  CallerInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                ICallerInfo infoDAL = CallerInfoFactory.Create(this.connStr, this.appCode);
                CallerInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public CallerInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               ICallerInfo infoDAL = CallerInfoFactory.Create(this.connStr, this.appCode);
        ...
        public CallerInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
