...
{
    public class LEDMatrixBLL
    {

        private string connStr;
        private string appCode;

        public  LEDMatrixBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               ILEDMatrix infoDAL = LEDMatrixFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" MatrixNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(LEDMatrix info)
        {
            try
            {
            if (!ValidateRepeat(info.sMatrixNo))
            {
                ILEDMatrix infoDAL = LEDMatrixFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(LEDMatrix info)
        {
            try
            {
                     ILEDMatrix  infoDAL = LEDMatrixFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  LEDMatrix GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                ILEDMatrix infoDAL = LEDMatrixFactory.Create(this.connStr, this.appCode);
                LEDMatrixCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public LEDMatrixCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               ILEDMatrix infoDAL = LEDMatrixFactory.Create(this.connStr, this.appCode);
        ...
        public LEDMatrixCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
