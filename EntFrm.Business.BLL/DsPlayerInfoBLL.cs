...
{
    public class DsPlayerInfoBLL
    {

        private string connStr;
        private string appCode;

        public  DsPlayerInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IDsPlayerInfo infoDAL = DsPlayerInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" PlayerNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(DsPlayerInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sPlayerNo))
            {
                IDsPlayerInfo infoDAL = DsPlayerInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(DsPlayerInfo info)
        {
            try
            {
                     IDsPlayerInfo  infoDAL = DsPlayerInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  DsPlayerInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IDsPlayerInfo infoDAL = DsPlayerInfoFactory.Create(this.connStr, this.appCode);
                DsPlayerInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public DsPlayerInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IDsPlayerInfo infoDAL = DsPlayerInfoFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "DsPlayerInfo";
        ...
        public DsPlayerInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
