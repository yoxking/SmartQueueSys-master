...
{
    public class DsPublishFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  DsPublishFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IDsPublishFlows infoDAL = DsPublishFlowsFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" PFlowNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(DsPublishFlows info)
        {
            try
            {
            if (!ValidateRepeat(info.sPFlowNo))
            {
                IDsPublishFlows infoDAL = DsPublishFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(DsPublishFlows info)
        {
            try
            {
                     IDsPublishFlows  infoDAL = DsPublishFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  DsPublishFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IDsPublishFlows infoDAL = DsPublishFlowsFactory.Create(this.connStr, this.appCode);
                DsPublishFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public DsPublishFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IDsPublishFlows infoDAL = DsPublishFlowsFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "DsPublishFlows";
        ...
        public DsPublishFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
