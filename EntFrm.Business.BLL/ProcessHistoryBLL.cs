...
namespace EntFrm.Business.BLL
{
    public class ProcessHistoryBLL
    {
        ...
        public  ProcessHistoryBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IProcessHistory infoDAL = ProcessHistoryFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(ProcessHistory info)
        {
            try
            {
        ...
        public  bool UpdateRecord(ProcessHistory info)
        {
            try
            {
                     IProcessHistory  infoDAL = ProcessHistoryFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  ProcessHistory GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IProcessHistory infoDAL = ProcessHistoryFactory.Create(this.connStr, this.appCode);
                ProcessHistoryCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public ProcessHistoryCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IProcessHistory infoDAL = ProcessHistoryFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "ProcessHistory";
        ...
        public ProcessHistoryCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
