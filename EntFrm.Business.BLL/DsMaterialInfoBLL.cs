...
{
    public class DsMaterialInfoBLL
    {

        private string connStr;
        private string appCode;

        public  DsMaterialInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IDsMaterialInfo infoDAL = DsMaterialInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" MatNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(DsMaterialInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sMatNo))
            {
                IDsMaterialInfo infoDAL = DsMaterialInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(DsMaterialInfo info)
        {
            try
            {
                     IDsMaterialInfo  infoDAL = DsMaterialInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  DsMaterialInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IDsMaterialInfo infoDAL = DsMaterialInfoFactory.Create(this.connStr, this.appCode);
                DsMaterialInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public DsMaterialInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IDsMaterialInfo infoDAL = DsMaterialInfoFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "DsMaterialInfo";
        ...
        public DsMaterialInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
