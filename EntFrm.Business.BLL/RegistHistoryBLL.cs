...
{
    public class RegistHistoryBLL
    {

        private string connStr;
        private string appCode;

        public  RegistHistoryBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IRegistHistory infoDAL = RegistHistoryFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(RegistHistory info)
        {
            try
            {
        ...
        public  bool UpdateRecord(RegistHistory info)
        {
            try
            {
                     IRegistHistory  infoDAL = RegistHistoryFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  RegistHistory GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IRegistHistory infoDAL = RegistHistoryFactory.Create(this.connStr, this.appCode);
                RegistHistoryCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public RegistHistoryCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IRegistHistory infoDAL = RegistHistoryFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "RegistHistory";
        ...
        public RegistHistoryCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
