...
{
    public class EvaluateFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  EvaluateFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IEvaluateFlows infoDAL = EvaluateFlowsFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" EFlowNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(EvaluateFlows info)
        {
            try
            {
            if (!ValidateRepeat(info.sEFlowNo))
            {
                IEvaluateFlows infoDAL = EvaluateFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(EvaluateFlows info)
        {
            try
            {
                     IEvaluateFlows  infoDAL = EvaluateFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  EvaluateFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IEvaluateFlows infoDAL = EvaluateFlowsFactory.Create(this.connStr, this.appCode);
                EvaluateFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public EvaluateFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IEvaluateFlows infoDAL = EvaluateFlowsFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "EvaluateFlows";
        ...
        public EvaluateFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
