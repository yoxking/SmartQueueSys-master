...
{
    public class ViewTicketFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  ViewTicketFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IViewTicketFlows infoDAL = ViewTicketFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(ViewTicketFlows info)
        {
            try
            {
        ...
        public  bool UpdateRecord(ViewTicketFlows info)
        {
            try
            {
                     IViewTicketFlows  infoDAL = ViewTicketFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  ViewTicketFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IViewTicketFlows infoDAL = ViewTicketFlowsFactory.Create(this.connStr, this.appCode);
                ViewTicketFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public ViewTicketFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IViewTicketFlows infoDAL = ViewTicketFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public ViewTicketFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
