...
{
    public class DsHrtbeatFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  DsHrtbeatFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IDsHrtbeatFlows infoDAL = DsHrtbeatFlowsFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" BFlowNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(DsHrtbeatFlows info)
        {
            try
            {
            if (!ValidateRepeat(info.sBFlowNo))
            {
                IDsHrtbeatFlows infoDAL = DsHrtbeatFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(DsHrtbeatFlows info)
        {
            try
            {
                     IDsHrtbeatFlows  infoDAL = DsHrtbeatFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  DsHrtbeatFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IDsHrtbeatFlows infoDAL = DsHrtbeatFlowsFactory.Create(this.connStr, this.appCode);
                DsHrtbeatFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public DsHrtbeatFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IDsHrtbeatFlows infoDAL = DsHrtbeatFlowsFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "DsHrtbeatFlows";
        ...
        public DsHrtbeatFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
