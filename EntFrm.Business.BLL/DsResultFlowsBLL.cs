...
{
    public class DsResultFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  DsResultFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IDsResultFlows infoDAL = DsResultFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(DsResultFlows info)
        {
            try
            {
        ...
        public  bool UpdateRecord(DsResultFlows info)
        {
            try
            {
                     IDsResultFlows  infoDAL = DsResultFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  DsResultFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IDsResultFlows infoDAL = DsResultFlowsFactory.Create(this.connStr, this.appCode);
                DsResultFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public DsResultFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IDsResultFlows infoDAL = DsResultFlowsFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "DsResultFlows";
        ...
        public DsResultFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
