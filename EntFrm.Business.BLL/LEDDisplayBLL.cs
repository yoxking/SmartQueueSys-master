...
{
    public class LEDDisplayBLL
    {

        private string connStr;
        private string appCode;

        public  LEDDisplayBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               ILEDDisplay infoDAL = LEDDisplayFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" DisplayNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(LEDDisplay info)
        {
            try
            {
            if (!ValidateRepeat(info.sDisplayNo))
            {
                ILEDDisplay infoDAL = LEDDisplayFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(LEDDisplay info)
        {
            try
            {
                     ILEDDisplay  infoDAL = LEDDisplayFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  LEDDisplay GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                ILEDDisplay infoDAL = LEDDisplayFactory.Create(this.connStr, this.appCode);
                LEDDisplayCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public LEDDisplayCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               ILEDDisplay infoDAL = LEDDisplayFactory.Create(this.connStr, this.appCode);
        ...
        public LEDDisplayCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
