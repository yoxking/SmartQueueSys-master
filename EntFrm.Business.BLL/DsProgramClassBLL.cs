...
{
    public class DsProgramClassBLL
    {

        private string connStr;
        private string appCode;

        public  DsProgramClassBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IDsProgramClass infoDAL = DsProgramClassFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(DsProgramClass info)
        {
            try
            {
        ...
        public  bool UpdateRecord(DsProgramClass info)
        {
            try
            {
                     IDsProgramClass  infoDAL = DsProgramClassFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  DsProgramClass GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IDsProgramClass infoDAL = DsProgramClassFactory.Create(this.connStr, this.appCode);
                DsProgramClassCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public DsProgramClassCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IDsProgramClass infoDAL = DsProgramClassFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "DsProgramClass";
        ...
        public DsProgramClassCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
