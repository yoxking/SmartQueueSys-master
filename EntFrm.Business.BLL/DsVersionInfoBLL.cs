...
{
    public class DsVersionInfoBLL
    {

        private string connStr;
        private string appCode;

        public  DsVersionInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IDsVersionInfo infoDAL = DsVersionInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" VerNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(DsVersionInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sVerNo))
            {
                IDsVersionInfo infoDAL = DsVersionInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(DsVersionInfo info)
        {
            try
            {
                     IDsVersionInfo  infoDAL = DsVersionInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  DsVersionInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IDsVersionInfo infoDAL = DsVersionInfoFactory.Create(this.connStr, this.appCode);
                DsVersionInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public DsVersionInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IDsVersionInfo infoDAL = DsVersionInfoFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "DsVersionInfo";
        ...
        public DsVersionInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
