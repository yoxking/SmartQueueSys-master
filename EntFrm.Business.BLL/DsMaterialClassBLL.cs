...
{
    public class DsMaterialClassBLL
    {

        private string connStr;
        private string appCode;

        public  DsMaterialClassBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IDsMaterialClass infoDAL = DsMaterialClassFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(DsMaterialClass info)
        {
            try
            {
        ...
        public  bool UpdateRecord(DsMaterialClass info)
        {
            try
            {
                     IDsMaterialClass  infoDAL = DsMaterialClassFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  DsMaterialClass GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IDsMaterialClass infoDAL = DsMaterialClassFactory.Create(this.connStr, this.appCode);
                DsMaterialClassCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public DsMaterialClassCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IDsMaterialClass infoDAL = DsMaterialClassFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "DsMaterialClass";
        ...
        public DsMaterialClassCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
