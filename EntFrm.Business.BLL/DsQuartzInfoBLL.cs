...
{
    public class DsQuartzInfoBLL
    {

        private string connStr;
        private string appCode;

        public  DsQuartzInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IDsQuartzInfo infoDAL = DsQuartzInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" QuartzNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(DsQuartzInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sQuartzNo))
            {
                IDsQuartzInfo infoDAL = DsQuartzInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(DsQuartzInfo info)
        {
            try
            {
                     IDsQuartzInfo  infoDAL = DsQuartzInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  DsQuartzInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IDsQuartzInfo infoDAL = DsQuartzInfoFactory.Create(this.connStr, this.appCode);
                DsQuartzInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public DsQuartzInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IDsQuartzInfo infoDAL = DsQuartzInfoFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "DsQuartzInfo";
        ...
        public DsQuartzInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
