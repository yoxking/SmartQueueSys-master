...
{
    public class ViewRegistFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  ViewRegistFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IViewRegistFlows infoDAL = ViewRegistFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(ViewRegistFlows info)
        {
            try
            {
        ...
        public  bool UpdateRecord(ViewRegistFlows info)
        {
            try
            {
                     IViewRegistFlows  infoDAL = ViewRegistFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  ViewRegistFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IViewRegistFlows infoDAL = ViewRegistFlowsFactory.Create(this.connStr, this.appCode);
                ViewRegistFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public ViewRegistFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IViewRegistFlows infoDAL = ViewRegistFlowsFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "ViewRegistFlows";
        ...
        public ViewRegistFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
