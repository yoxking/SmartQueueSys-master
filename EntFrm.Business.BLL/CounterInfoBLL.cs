...
{
    public class CounterInfoBLL
    {

        private string connStr;
        private string appCode;

        public  CounterInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               ICounterInfo infoDAL = CounterInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" CounterNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(CounterInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sCounterNo))
            {
                ICounterInfo infoDAL = CounterInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(CounterInfo info)
        {
            try
            {
                     ICounterInfo  infoDAL = CounterInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  CounterInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                ICounterInfo infoDAL = CounterInfoFactory.Create(this.connStr, this.appCode);
                CounterInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public CounterInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               ICounterInfo infoDAL = CounterInfoFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "CounterInfo";
        ...
        public CounterInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
