...
{
    public class DsProgramInfoBLL
    {

        private string connStr;
        private string appCode;

        public  DsProgramInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IDsProgramInfo infoDAL = DsProgramInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" ProgmNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(DsProgramInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sProgmNo))
            {
                IDsProgramInfo infoDAL = DsProgramInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(DsProgramInfo info)
        {
            try
            {
                     IDsProgramInfo  infoDAL = DsProgramInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  DsProgramInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IDsProgramInfo infoDAL = DsProgramInfoFactory.Create(this.connStr, this.appCode);
                DsProgramInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public DsProgramInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IDsProgramInfo infoDAL = DsProgramInfoFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "DsProgramInfo";
        ...
        public DsProgramInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
