...
{
    public class RegistFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  RegistFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IRegistFlows infoDAL = RegistFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(RegistFlows info)
        {
            try
            {
        ...
        public  bool UpdateRecord(RegistFlows info)
        {
            try
            {
                     IRegistFlows  infoDAL = RegistFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  RegistFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IRegistFlows infoDAL = RegistFlowsFactory.Create(this.connStr, this.appCode);
                RegistFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public RegistFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IRegistFlows infoDAL = RegistFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public RegistFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
