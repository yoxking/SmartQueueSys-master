...
{
    public class ProcessFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  ProcessFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IProcessFlows infoDAL = ProcessFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(ProcessFlows info)
        {
            try
            {
        ...
        public  bool UpdateRecord(ProcessFlows info)
        {
            try
            {
                     IProcessFlows  infoDAL = ProcessFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  ProcessFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IProcessFlows infoDAL = ProcessFlowsFactory.Create(this.connStr, this.appCode);
                ProcessFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public ProcessFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IProcessFlows infoDAL = ProcessFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public ProcessFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
