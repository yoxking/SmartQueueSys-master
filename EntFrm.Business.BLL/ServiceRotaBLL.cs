...
{
    public class ServiceRotaBLL
    {

        private string connStr;
        private string appCode;

        public  ServiceRotaBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IServiceRota infoDAL = ServiceRotaFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(ServiceRota info)
        {
            try
            {
        ...
        public  bool UpdateRecord(ServiceRota info)
        {
            try
            {
                     IServiceRota  infoDAL = ServiceRotaFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  ServiceRota GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IServiceRota infoDAL = ServiceRotaFactory.Create(this.connStr, this.appCode);
                ServiceRotaCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public ServiceRotaCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IServiceRota infoDAL = ServiceRotaFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "ServiceRota";
        ...
        public ServiceRotaCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
