...
{
    public class DsDwloadFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  DsDwloadFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IDsDwloadFlows infoDAL = DsDwloadFlowsFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" DFlowNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(DsDwloadFlows info)
        {
            try
            {
            if (!ValidateRepeat(info.sDFlowNo))
            {
                IDsDwloadFlows infoDAL = DsDwloadFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(DsDwloadFlows info)
        {
            try
            {
                     IDsDwloadFlows  infoDAL = DsDwloadFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  DsDwloadFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IDsDwloadFlows infoDAL = DsDwloadFlowsFactory.Create(this.connStr, this.appCode);
                DsDwloadFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public DsDwloadFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IDsDwloadFlows infoDAL = DsDwloadFlowsFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "DsDwloadFlows";
        ...
        public DsDwloadFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
