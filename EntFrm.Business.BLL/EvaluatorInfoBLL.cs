...
namespace EntFrm.Business.BLL
{
    public class EvaluatorInfoBLL
    {
        ...
        public  EvaluatorInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IEvaluatorInfo infoDAL = EvaluatorInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" EvalorNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(EvaluatorInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sEvalorNo))
            {
                IEvaluatorInfo infoDAL = EvaluatorInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(EvaluatorInfo info)
        {
            try
            {
                     IEvaluatorInfo  infoDAL = EvaluatorInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  EvaluatorInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IEvaluatorInfo infoDAL = EvaluatorInfoFactory.Create(this.connStr, this.appCode);
                EvaluatorInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public EvaluatorInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IEvaluatorInfo infoDAL = EvaluatorInfoFactory.Create(this.connStr, this.appCode);
            ...
            s_model.sOrderType = "desc";
             s_model.sTableName = "EvaluatorInfo";
        ...
        public EvaluatorInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
