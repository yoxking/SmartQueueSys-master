...
{
    public class SWorkFlowsBLL
    {

        private string connStr;
        private string appCode;

        public  SWorkFlowsBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               ISWorkFlows infoDAL = SWorkFlowsFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" WFlowNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(SWorkFlows info)
        {
            try
            {
            if (!ValidateRepeat(info.sWFlowNo))
            {
                ISWorkFlows infoDAL = SWorkFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(SWorkFlows info)
        {
            try
            {
                     ISWorkFlows  infoDAL = SWorkFlowsFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  SWorkFlows GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                ISWorkFlows infoDAL = SWorkFlowsFactory.Create(this.connStr, this.appCode);
                SWorkFlowsCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public SWorkFlowsCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               ISWorkFlows infoDAL = SWorkFlowsFactory.Create(this.connStr, this.appCode);
        ...
        public SWorkFlowsCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
