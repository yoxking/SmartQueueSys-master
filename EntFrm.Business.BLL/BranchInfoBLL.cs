...
namespace EntFrm.Business.BLL
{
    public class BranchInfoBLL
    {
        ...
        public  BranchInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IBranchInfo infoDAL = BranchInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" BranchNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(BranchInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sBranchNo))
            {
                IBranchInfo infoDAL = BranchInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(BranchInfo info)
        {
            try
            {
                     IBranchInfo  infoDAL = BranchInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  BranchInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IBranchInfo infoDAL = BranchInfoFactory.Create(this.connStr, this.appCode);
                BranchInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public BranchInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IBranchInfo infoDAL = BranchInfoFactory.Create(this.connStr, this.appCode);
        ...
        public BranchInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
