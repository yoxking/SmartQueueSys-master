...
{
    public class VoiceInfoBLL
    {

        private string connStr;
        private string appCode;

        public  VoiceInfoBLL(string sConnStr, string sAppCode)
        {
        ...
        if (!string.IsNullOrEmpty(sNo))
        {
           try
            {
               IVoiceInfo infoDAL = VoiceInfoFactory.Create(this.connStr, this.appCode);
               if (infoDAL.GetCountByCondition(" TtsNo ='" + sNo + "'") > 0)
               {
                   return true;
               }
               return false;
        ...
        public  bool AddNewRecord(VoiceInfo info)
        {
            try
            {
            if (!ValidateRepeat(info.sTtsNo))
            {
                IVoiceInfo infoDAL = VoiceInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool UpdateRecord(VoiceInfo info)
        {
            try
            {
                     IVoiceInfo  infoDAL = VoiceInfoFactory.Create(this.connStr, this.appCode);
...
public bool HardDeleteByCondition(string sCondition)
{
    ...
    catch (Exception ex)
    {
        ...
        public  VoiceInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                IVoiceInfo infoDAL = VoiceInfoFactory.Create(this.connStr, this.appCode);
                VoiceInfoCollections infoList = infoDAL.GetRecordsByNo(sNo);
        ...
        public VoiceInfoCollections GetRecordsByPaging(ref int pageCount, int pageIndex, int pageSize, string condition)
        {
           try
           {
               IVoiceInfo infoDAL = VoiceInfoFactory.Create(this.connStr, this.appCode);
        ...
        public VoiceInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model != null)
            {
                try
