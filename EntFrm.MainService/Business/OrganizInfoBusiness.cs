...
{
    public class OrganizInfoBusiness
    {

        private volatile static OrganizInfoBusiness _instance = null;
        private static readonly object lockHelper = new object();
        public static OrganizInfoBusiness CreateInstance()
        {
            if (_instance == null)
            {
                lock (lockHelper)
                {
                    if (_instance == null)
                        _instance = new OrganizInfoBusiness();
                }
            }
            return _instance;
        }
        public string GetAllRecords()
        {
            try
            {
                ...
                OrganizInfoBLL infoBoss = new OrganizInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                OrganizInfoCollections infoColl = infoBoss.GetRecordsByPaging(ref count, 1, 100, sWhere);
                if (infoColl != null)
                {
        ...
        public string GetRecordByNo(string sNo)
        {
            try
            {
                string sResult = "";
                OrganizInfoBLL infoBoss = new OrganizInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                OrganizInfo info = infoBoss.GetRecordByNo(sNo);
                if (info != null)
                {
        ...
        public string GetRecordsByPaging(string pageIndex, string pageSize, string condition)
        {
            try
            {
                ...
                OrganizInfoBLL infoBoss = new OrganizInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                OrganizInfoCollections infoColl = infoBoss.GetRecordsByPaging(ref count, int.Parse(pageIndex), int.Parse(pageSize), sWhere);
                if (infoColl != null)
                {
                    sResult = JsonConvert.SerializeObject(infoColl, Formatting.None);
                }
        ...
        public string PostRecord(string sInfo)
        {
            try
            {
                ...
                OrganizInfo info1 = JsonConvert.DeserializeObject<OrganizInfo>(sInfo);
                if (info1 != null)
                {
                    OrganizInfoBLL infoBoss = new OrganizInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                    OrganizInfo info2 = infoBoss.GetRecordByNo(info1.sOrganizNo);
                    if (info2 == null)
                    {
                        info2 = info1;
                        info2.sOrganizNo = CommonHelper.Get_New12ByteGuid();
