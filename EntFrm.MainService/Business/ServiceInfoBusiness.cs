...
{
    public class ServiceInfoBusiness
    {

        private volatile static ServiceInfoBusiness _instance = null;
        private static readonly object lockHelper = new object();
        public static ServiceInfoBusiness CreateInstance()
        {
            if (_instance == null)
            {
                lock (lockHelper)
                {
                    if (_instance == null)
                        _instance = new ServiceInfoBusiness();
                }
            }
            return _instance;
        }
        ...
        public string GetRecordByNo(string sNo)
        {
            try
            {
                string sResult = "";
                ServiceInfoBLL infoBoss = new ServiceInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                ServiceInfo info = infoBoss.GetRecordByNo(sNo);
                if (info != null)
                {
        ...
        public string GetRecordsByPaging(string pageIndex, string pageSize, string condition)
        {
            try
            {
                ...
                ServiceInfoBLL infoBoss = new ServiceInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                ServiceInfoCollections infoColl = infoBoss.GetRecordsByPaging(ref count, int.Parse(pageIndex), int.Parse(pageSize), sWhere);
                if (infoColl != null)
                {
                    sResult = JsonConvert.SerializeObject(infoColl, Formatting.None);
                }
        ...
        public string PostRecord(string sInfo)
        {
            try
            {
                ...
                ServiceInfo info1 = JsonConvert.DeserializeObject<ServiceInfo>(sInfo);
                if (info1 != null)
                {
                    ServiceInfoBLL infoBoss = new ServiceInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                    ServiceInfo info2 = infoBoss.GetRecordByNo(info1.sServiceNo);
                    if (info2 == null)
                    {
                        info2 = info1;
                        info2.sServiceNo = CommonHelper.Get_New12ByteGuid();
