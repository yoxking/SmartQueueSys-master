...
{
    public class TicketFlowsBusiness
    {

    private volatile static TicketFlowsBusiness _instance = null;
    private static readonly object lockHelper = new object();
    public static TicketFlowsBusiness CreateInstance()
    {
    if (_instance == null)
    {
    lock (lockHelper)
    {
    if (_instance == null)
      _instance = new TicketFlowsBusiness();
    }
    }
    return _instance;
    }
        public  string GetAllRecords()
        {
            try
            {
               ...
               TicketFlowsBLL infoBoss = new TicketFlowsBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
               TicketFlowsCollections infoColl = infoBoss.GetRecordsByPaging(ref count, 1, 100, sWhere);
               if (infoColl != null)
               {
        ...
        public string GetRecordByNo(string sNo)
        {
                try
                {
                  string sResult = "";
                  TicketFlowsBLL infoBoss = new TicketFlowsBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                 TicketFlows info = infoBoss.GetRecordByNo(sNo);
                 if (info != null)
                 {
        ...
        try
         {
            ...
            TicketFlowsBLL infoBoss = new TicketFlowsBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
            TicketFlowsCollections infoColl = infoBoss.GetRecordsByPaging(ref count, int.Parse(pageIndex), int.Parse(pageSize), sWhere);
            if (infoColl != null)
            {
              sResult = JsonConvert.SerializeObject(infoColl, Formatting.None);
            }
        ...
        public  string PostRecord(string sInfo)
        {
            try
            {
              ...
              TicketFlows info1 = JsonConvert.DeserializeObject<TicketFlows>(sInfo);
              if (info1 != null)
              {
                 TicketFlowsBLL infoBoss = new TicketFlowsBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                 TicketFlows info2 = infoBoss.GetRecordByNo(info1.sTFlowNo);
                 if (info2 == null)
                 {
                    info2 = info1;
                    info2.sTFlowNo = CommonHelper.Get_New12ByteGuid();
