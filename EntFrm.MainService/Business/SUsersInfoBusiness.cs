...
{
    public class SUsersInfoBusiness
    {

        private volatile static SUsersInfoBusiness _instance = null;
        private static readonly object lockHelper = new object();
        public static SUsersInfoBusiness CreateInstance()
        {
            if (_instance == null)
            {
                lock (lockHelper)
                {
                    if (_instance == null)
                        _instance = new SUsersInfoBusiness();
                }
            }
            return _instance;
        }
        public string GetAllRecords()
        {
            try
            {
                ...
                SUsersInfoBLL infoBoss = new SUsersInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                SUsersInfoCollections infoColl = infoBoss.GetRecordsByPaging(ref count, 1, 100, sWhere);
                if (infoColl != null)
                {
        ...
        public string GetRecordByNo(string sNo)
        {
            try
            {
                string sResult = "";
                SUsersInfoBLL infoBoss = new SUsersInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                SUsersInfo info = infoBoss.GetRecordByNo(sNo);
                if (info != null)
                {
        ...
        public string GetRecordsByPaging(string pageIndex, string pageSize, string condition)
        {
            try
            {
                ...
                SUsersInfoBLL infoBoss = new SUsersInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                SUsersInfoCollections infoColl = infoBoss.GetRecordsByPaging(ref count, int.Parse(pageIndex), int.Parse(pageSize), sWhere);
                if (infoColl != null)
                {
                    sResult = JsonConvert.SerializeObject(infoColl, Formatting.None);
                }
        ...
        public string PostRecord(string sInfo)
        {
            try
            {
                ...
                SUsersInfo info1 = JsonConvert.DeserializeObject<SUsersInfo>(sInfo);
                if (info1 != null)
                {
                    SUsersInfoBLL infoBoss = new SUsersInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                    SUsersInfo info2 = infoBoss.GetRecordByNo(info1.sSuNo);
                    if (info2 == null)
                    {
                        info2 = info1;
                        info2.sSuNo = CommonHelper.Get_New12ByteGuid();
