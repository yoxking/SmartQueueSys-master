...
{
    public class RUsersInfoBusiness
    {

        private volatile static RUsersInfoBusiness _instance = null;
        private static readonly object lockHelper = new object();
        public static RUsersInfoBusiness CreateInstance()
        {
            if (_instance == null)
            {
                lock (lockHelper)
                {
                    if (_instance == null)
                        _instance = new RUsersInfoBusiness();
                }
            }
            return _instance;
        }
        public string GetAllRecords()
        {
            try
            {
                ...
                RUsersInfoCollections infoColl = infoBoss.GetRecordsByPaging(ref count, 1, 100, sWhere);
                if (infoColl != null)
                {
        ...
        public string GetRecordByNo(string sNo)
        {
            try
            {
                string sResult = "";
                RUsersInfoBLL infoBoss = new RUsersInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                RUsersInfo info = infoBoss.GetRecordByNo(sNo);
                if (info != null)
                {
        ...
        public string GetRecordsByPaging(string pageIndex, string pageSize, string condition)
        {
            try
            {
                ...
                RUsersInfoBLL infoBoss = new RUsersInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                RUsersInfoCollections infoColl = infoBoss.GetRecordsByPaging(ref count, int.Parse(pageIndex), int.Parse(pageSize), sWhere);
                if (infoColl != null)
                {
                    sResult = JsonConvert.SerializeObject(infoColl, Formatting.None);
                }
        ...
        public string PostRecord(string sInfo)
        {
            try
            {
                ...
                RUsersInfo info1 = JsonConvert.DeserializeObject<RUsersInfo>(sInfo);
                if (info1 != null)
                {
                    RUsersInfoBLL infoBoss = new RUsersInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                    RUsersInfo info2 = infoBoss.GetRecordByNo(info1.sRUserNo);
                    if (info2 == null)
                    {
                        info2 = info1;
                        info2.sRUserNo = CommonHelper.Get_New12ByteGuid();
