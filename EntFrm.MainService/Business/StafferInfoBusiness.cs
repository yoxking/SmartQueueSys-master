...
{
    public class StafferInfoBusiness
    {

        private volatile static StafferInfoBusiness _instance = null;
        private static readonly object lockHelper = new object();
        public static StafferInfoBusiness CreateInstance()
        {
            if (_instance == null)
            {
                lock (lockHelper)
                {
                    if (_instance == null)
                        _instance = new StafferInfoBusiness();
                }
            }
            return _instance;
        }
        ...
        public string GetRecordByNo(string sNo)
        {
            try
            {
                string sResult = "";
                StafferInfoBLL infoBoss = new StafferInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                StafferInfo info = infoBoss.GetRecordByNo(sNo);
                if (info != null)
                {
        ...
        public string GetRecordsByPaging(string pageIndex, string pageSize, string condition)
        {
            try
            {
                ...
                StafferInfoBLL infoBoss = new StafferInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                StafferInfoCollections infoColl = infoBoss.GetRecordsByPaging(ref count, int.Parse(pageIndex), int.Parse(pageSize), sWhere);
                if (infoColl != null)
                {
                    sResult = JsonConvert.SerializeObject(infoColl, Formatting.None);
                }
        ...
        public string PostRecord(string sInfo)
        {
            try
            {
                ...
                StafferInfo info1 = JsonConvert.DeserializeObject<StafferInfo>(sInfo);
                if (info1 != null)
                {
                    StafferInfoBLL infoBoss = new StafferInfoBLL(IUserContext.GetConnStr(), IUserContext.GetAppCode());
                    StafferInfo info2 = infoBoss.GetRecordByNo(info1.sStafferNo);
                    if (info2 == null)
                    {
                        info2 = info1;
                        info2.sStafferNo = CommonHelper.Get_New12ByteGuid();
