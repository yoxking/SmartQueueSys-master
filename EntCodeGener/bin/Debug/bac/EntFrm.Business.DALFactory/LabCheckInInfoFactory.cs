...
{
  public sealed class LabCheckInInfoFactory
  {
        public static ILabCheckInInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".LabCheckInInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (ILabCheckInInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
