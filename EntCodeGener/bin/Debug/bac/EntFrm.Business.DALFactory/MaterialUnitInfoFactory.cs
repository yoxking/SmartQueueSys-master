...
{
  public sealed class MaterialUnitInfoFactory
  {
        public static IMaterialUnitInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".MaterialUnitInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IMaterialUnitInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
