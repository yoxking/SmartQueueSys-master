...
{
  public sealed class DeviceInventoryFactory
  {
        public static IDeviceInventory Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".DeviceInventoryDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IDeviceInventory)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
