...
{
  public sealed class TermInfoFactory
  {
        public static ITermInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".TermInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (ITermInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
