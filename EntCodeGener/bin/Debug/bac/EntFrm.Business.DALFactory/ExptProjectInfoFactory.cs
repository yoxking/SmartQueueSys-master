...
{
  public sealed class ExptProjectInfoFactory
  {
        public static IExptProjectInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".ExptProjectInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IExptProjectInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
