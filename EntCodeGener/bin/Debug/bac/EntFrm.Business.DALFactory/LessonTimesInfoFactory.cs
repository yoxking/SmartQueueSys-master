...
{
  public sealed class LessonTimesInfoFactory
  {
        public static ILessonTimesInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".LessonTimesInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (ILessonTimesInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
