...
{
  public sealed class CategoriesFactory
  {
        public static ICategories Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".CategoriesDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (ICategories)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
