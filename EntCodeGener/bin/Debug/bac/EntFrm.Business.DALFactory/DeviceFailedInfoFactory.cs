...
{
  public sealed class DeviceFailedInfoFactory
  {
        public static IDeviceFailedInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".DeviceFailedInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IDeviceFailedInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
