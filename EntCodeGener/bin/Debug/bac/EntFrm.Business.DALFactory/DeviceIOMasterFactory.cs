...
{
  public sealed class DeviceIOMasterFactory
  {
        public static IDeviceIOMaster Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".DeviceIOMasterDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IDeviceIOMaster)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
