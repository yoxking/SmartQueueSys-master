...
{
  public sealed class ProfessionInfoFactory
  {
        public static IProfessionInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".ProfessionInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IProfessionInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
