...
{
  public sealed class MaterialInfoFactory
  {
        public static IMaterialInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".MaterialInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IMaterialInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
