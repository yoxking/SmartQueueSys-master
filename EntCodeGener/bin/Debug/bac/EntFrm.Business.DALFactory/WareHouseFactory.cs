...
namespace EntFrm.Business.DALFactory
{
  ...
  {
        public static IWareHouse Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".WareHouseDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IWareHouse)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
