...
{
  public sealed class DeviceGuaranteeInfoFactory
  {
        public static IDeviceGuaranteeInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".DeviceGuaranteeInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IDeviceGuaranteeInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
