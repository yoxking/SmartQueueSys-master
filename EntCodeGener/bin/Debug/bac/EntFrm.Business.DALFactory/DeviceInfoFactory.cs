...
{
  public sealed class DeviceInfoFactory
  {
        public static IDeviceInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".DeviceInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IDeviceInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
