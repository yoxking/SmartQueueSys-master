...
{
  public sealed class OutlineInfoFactory
  {
        public static IOutlineInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".OutlineInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IOutlineInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
