...
{
  public sealed class MaterialTypeInfoFactory
  {
        public static IMaterialTypeInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".MaterialTypeInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IMaterialTypeInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
