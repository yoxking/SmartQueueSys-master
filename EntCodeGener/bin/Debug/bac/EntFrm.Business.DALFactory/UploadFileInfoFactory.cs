...
{
  public sealed class UploadFileInfoFactory
  {
        public static IUploadFileInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".UploadFileInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IUploadFileInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
