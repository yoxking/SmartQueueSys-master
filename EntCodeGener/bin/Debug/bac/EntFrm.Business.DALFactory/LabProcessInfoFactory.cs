...
{
  public sealed class LabProcessInfoFactory
  {
        public static ILabProcessInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".LabProcessInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (ILabProcessInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
