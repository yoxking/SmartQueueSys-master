...
{
  public sealed class MaterialIOMasterFactory
  {
        public static IMaterialIOMaster Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".MaterialIOMasterDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IMaterialIOMaster)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
