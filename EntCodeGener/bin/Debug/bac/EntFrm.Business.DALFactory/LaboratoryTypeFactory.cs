...
{
  public sealed class LaboratoryTypeFactory
  {
        public static ILaboratoryType Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".LaboratoryTypeDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (ILaboratoryType)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
