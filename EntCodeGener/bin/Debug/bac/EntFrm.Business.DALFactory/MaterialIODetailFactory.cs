...
{
  public sealed class MaterialIODetailFactory
  {
        public static IMaterialIODetail Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".MaterialIODetailDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IMaterialIODetail)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
