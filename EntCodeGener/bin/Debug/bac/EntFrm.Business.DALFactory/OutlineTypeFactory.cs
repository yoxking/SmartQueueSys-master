...
{
  public sealed class OutlineTypeFactory
  {
        public static IOutlineType Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".OutlineTypeDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IOutlineType)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
