...
{
  public sealed class LabOpeningInfoFactory
  {
        public static ILabOpeningInfo Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".LabOpeningInfoDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (ILabOpeningInfo)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
