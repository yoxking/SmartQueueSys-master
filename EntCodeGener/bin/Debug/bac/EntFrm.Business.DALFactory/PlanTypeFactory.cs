...
{
  public sealed class PlanTypeFactory
  {
        public static IPlanType Create(string sUrl,string sAppCode) 
        {
          try
          {
           string path = ConfigHelper.Current.ParamsConfig.ParamsConfigItems.Single(n => n.Key == "BusinessDAL").Value;
           string className = path + ".PlanTypeDAL," + path;

           Type typeofControl = Type.GetType(className,true,true);
           return (IPlanType)Activator.CreateInstance(typeofControl, new object[] { sUrl, sAppCode });
          }
          catch (Exception ex)
          {
