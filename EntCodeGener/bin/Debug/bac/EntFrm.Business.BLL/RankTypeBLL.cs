...
namespace EntFrm.Business.BLL
{
    public class RankTypeBLL
    {
        ...
        public  RankTypeBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(RankType info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(RankType  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IRankType  infoDAL = RankTypeFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(RankType info)
        {
            try
            {
        ...
        public  bool UpdateRecord(RankType info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  RankType GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IRankType  infoDAL = RankTypeFactory.Create(this.connStr, this.appCode);
                  RankTypeCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  RankTypeCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IRankType  infoDAL = RankTypeFactory.Create(this.connStr, this.appCode);
        ...
        public  RankTypeCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
