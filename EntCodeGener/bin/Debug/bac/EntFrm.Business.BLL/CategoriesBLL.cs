...
namespace EntFrm.Business.BLL
{
    public class CategoriesBLL
    {
        ...
        public  CategoriesBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(Categories info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(Categories  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     ICategories  infoDAL = CategoriesFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(Categories info)
        {
            try
            {
        ...
        public  bool UpdateRecord(Categories info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  Categories GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  ICategories  infoDAL = CategoriesFactory.Create(this.connStr, this.appCode);
                  CategoriesCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  CategoriesCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             ICategories  infoDAL = CategoriesFactory.Create(this.connStr, this.appCode);
        ...
        public  CategoriesCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
