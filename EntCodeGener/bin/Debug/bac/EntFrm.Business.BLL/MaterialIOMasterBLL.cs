...
{
    public class MaterialIOMasterBLL
    {
        ...
        public  MaterialIOMasterBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(MaterialIOMaster info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(MaterialIOMaster  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IMaterialIOMaster  infoDAL = MaterialIOMasterFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(MaterialIOMaster info)
        {
            try
            {
        ...
        public  bool UpdateRecord(MaterialIOMaster info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  MaterialIOMaster GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IMaterialIOMaster  infoDAL = MaterialIOMasterFactory.Create(this.connStr, this.appCode);
                  MaterialIOMasterCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  MaterialIOMasterCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IMaterialIOMaster  infoDAL = MaterialIOMasterFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "MaterialIOMaster";
        ...
        public  MaterialIOMasterCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
