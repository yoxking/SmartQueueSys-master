...
namespace EntFrm.Business.BLL
{
    public class ProviderInfoBLL
    {
        ...
        public  ProviderInfoBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(ProviderInfo info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(ProviderInfo  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IProviderInfo  infoDAL = ProviderInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(ProviderInfo info)
        {
            try
            {
        ...
        public  bool UpdateRecord(ProviderInfo info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  ProviderInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IProviderInfo  infoDAL = ProviderInfoFactory.Create(this.connStr, this.appCode);
                  ProviderInfoCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  ProviderInfoCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IProviderInfo  infoDAL = ProviderInfoFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "ProviderInfo";
        ...
        public  ProviderInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
