...
namespace EntFrm.Business.BLL
{
    public class ProfessionInfoBLL
    {
        ...
        public  ProfessionInfoBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(ProfessionInfo info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(ProfessionInfo  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IProfessionInfo  infoDAL = ProfessionInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(ProfessionInfo info)
        {
            try
            {
        ...
        public  bool UpdateRecord(ProfessionInfo info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  ProfessionInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IProfessionInfo  infoDAL = ProfessionInfoFactory.Create(this.connStr, this.appCode);
                  ProfessionInfoCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  ProfessionInfoCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IProfessionInfo  infoDAL = ProfessionInfoFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "ProfessionInfo";
        ...
        public  ProfessionInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
