...
namespace EntFrm.Business.BLL
{
    public class AbstractBLL
    {
        ...
        public  AbstractBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(Abstract info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(Abstract  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IAbstract  infoDAL = AbstractFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(Abstract info)
        {
            try
            {
        ...
        public  bool UpdateRecord(Abstract info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  Abstract GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IAbstract  infoDAL = AbstractFactory.Create(this.connStr, this.appCode);
                  AbstractCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  AbstractCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IAbstract  infoDAL = AbstractFactory.Create(this.connStr, this.appCode);
        ...
        public  AbstractCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
