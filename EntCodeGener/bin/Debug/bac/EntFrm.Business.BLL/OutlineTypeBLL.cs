...
namespace EntFrm.Business.BLL
{
    public class OutlineTypeBLL
    {
        ...
        public  OutlineTypeBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(OutlineType info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(OutlineType  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IOutlineType  infoDAL = OutlineTypeFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(OutlineType info)
        {
            try
            {
        ...
        public  bool UpdateRecord(OutlineType info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  OutlineType GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IOutlineType  infoDAL = OutlineTypeFactory.Create(this.connStr, this.appCode);
                  OutlineTypeCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  OutlineTypeCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IOutlineType  infoDAL = OutlineTypeFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "OutlineType";
        ...
        public  OutlineTypeCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
