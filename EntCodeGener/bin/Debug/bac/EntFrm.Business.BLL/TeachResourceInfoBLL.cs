...
{
    public class TeachResourceInfoBLL
    {
        ...
        public  TeachResourceInfoBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(TeachResourceInfo info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(TeachResourceInfo  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     ITeachResourceInfo  infoDAL = TeachResourceInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(TeachResourceInfo info)
        {
            try
            {
        ...
        public  bool UpdateRecord(TeachResourceInfo info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  TeachResourceInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  ITeachResourceInfo  infoDAL = TeachResourceInfoFactory.Create(this.connStr, this.appCode);
                  TeachResourceInfoCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  TeachResourceInfoCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             ITeachResourceInfo  infoDAL = TeachResourceInfoFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "TeachResourceInfo";
        ...
        public  TeachResourceInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
