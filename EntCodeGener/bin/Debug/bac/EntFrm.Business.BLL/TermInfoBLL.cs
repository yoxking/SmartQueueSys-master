...
namespace EntFrm.Business.BLL
{
    public class TermInfoBLL
    {
        ...
        public  TermInfoBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(TermInfo info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(TermInfo  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     ITermInfo  infoDAL = TermInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(TermInfo info)
        {
            try
            {
        ...
        public  bool UpdateRecord(TermInfo info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  TermInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  ITermInfo  infoDAL = TermInfoFactory.Create(this.connStr, this.appCode);
                  TermInfoCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  TermInfoCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             ITermInfo  infoDAL = TermInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  TermInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
