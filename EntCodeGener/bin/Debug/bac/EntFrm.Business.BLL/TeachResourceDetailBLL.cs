...
{
    public class TeachResourceDetailBLL
    {
        ...
        public  TeachResourceDetailBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(TeachResourceDetail info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(TeachResourceDetail  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     ITeachResourceDetail  infoDAL = TeachResourceDetailFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(TeachResourceDetail info)
        {
            try
            {
        ...
        public  bool UpdateRecord(TeachResourceDetail info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  TeachResourceDetail GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  ITeachResourceDetail  infoDAL = TeachResourceDetailFactory.Create(this.connStr, this.appCode);
                  TeachResourceDetailCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  TeachResourceDetailCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             ITeachResourceDetail  infoDAL = TeachResourceDetailFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "TeachResourceDetail";
        ...
        public  TeachResourceDetailCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
