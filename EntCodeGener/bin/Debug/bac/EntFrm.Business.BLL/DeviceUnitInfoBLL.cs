...
{
    public class DeviceUnitInfoBLL
    {
        ...
        public  DeviceUnitInfoBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(DeviceUnitInfo info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(DeviceUnitInfo  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IDeviceUnitInfo  infoDAL = DeviceUnitInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(DeviceUnitInfo info)
        {
            try
            {
        ...
        public  bool UpdateRecord(DeviceUnitInfo info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  DeviceUnitInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IDeviceUnitInfo  infoDAL = DeviceUnitInfoFactory.Create(this.connStr, this.appCode);
                  DeviceUnitInfoCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  DeviceUnitInfoCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IDeviceUnitInfo  infoDAL = DeviceUnitInfoFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "DeviceUnitInfo";
        ...
        public  DeviceUnitInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
