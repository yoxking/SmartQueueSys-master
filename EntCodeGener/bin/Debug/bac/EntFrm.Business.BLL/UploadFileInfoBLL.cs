...
{
    public class UploadFileInfoBLL
    {
        ...
        public  UploadFileInfoBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(UploadFileInfo info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(UploadFileInfo  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IUploadFileInfo  infoDAL = UploadFileInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(UploadFileInfo info)
        {
            try
            {
        ...
        public  bool UpdateRecord(UploadFileInfo info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  UploadFileInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IUploadFileInfo  infoDAL = UploadFileInfoFactory.Create(this.connStr, this.appCode);
                  UploadFileInfoCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  UploadFileInfoCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IUploadFileInfo  infoDAL = UploadFileInfoFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "UploadFileInfo";
        ...
        public  UploadFileInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
