...
namespace EntFrm.Business.BLL
{
    public class WareHouseBLL
    {
        ...
        public  WareHouseBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(WareHouse info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(WareHouse  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IWareHouse  infoDAL = WareHouseFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(WareHouse info)
        {
            try
            {
        ...
        public  bool UpdateRecord(WareHouse info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  WareHouse GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IWareHouse  infoDAL = WareHouseFactory.Create(this.connStr, this.appCode);
                  WareHouseCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  WareHouseCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IWareHouse  infoDAL = WareHouseFactory.Create(this.connStr, this.appCode);
        ...
        public  WareHouseCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
