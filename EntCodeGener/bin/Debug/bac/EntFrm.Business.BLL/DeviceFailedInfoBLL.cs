...
{
    public class DeviceFailedInfoBLL
    {
        ...
        public  DeviceFailedInfoBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(DeviceFailedInfo info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(DeviceFailedInfo  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IDeviceFailedInfo  infoDAL = DeviceFailedInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(DeviceFailedInfo info)
        {
            try
            {
        ...
        public  bool UpdateRecord(DeviceFailedInfo info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  DeviceFailedInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IDeviceFailedInfo  infoDAL = DeviceFailedInfoFactory.Create(this.connStr, this.appCode);
                  DeviceFailedInfoCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  DeviceFailedInfoCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IDeviceFailedInfo  infoDAL = DeviceFailedInfoFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "DeviceFailedInfo";
        ...
        public  DeviceFailedInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
