...
{
    public class DeviceInventoryBLL
    {
        ...
        public  DeviceInventoryBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(DeviceInventory info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(DeviceInventory  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IDeviceInventory  infoDAL = DeviceInventoryFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(DeviceInventory info)
        {
            try
            {
        ...
        public  bool UpdateRecord(DeviceInventory info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  DeviceInventory GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IDeviceInventory  infoDAL = DeviceInventoryFactory.Create(this.connStr, this.appCode);
                  DeviceInventoryCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  DeviceInventoryCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IDeviceInventory  infoDAL = DeviceInventoryFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "DeviceInventory";
        ...
        public  DeviceInventoryCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
