...
{
    public class ExptProjectInfoBLL
    {
        ...
        public  ExptProjectInfoBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(ExptProjectInfo info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(ExptProjectInfo  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IExptProjectInfo  infoDAL = ExptProjectInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(ExptProjectInfo info)
        {
            try
            {
        ...
        public  bool UpdateRecord(ExptProjectInfo info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  ExptProjectInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IExptProjectInfo  infoDAL = ExptProjectInfoFactory.Create(this.connStr, this.appCode);
                  ExptProjectInfoCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  ExptProjectInfoCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IExptProjectInfo  infoDAL = ExptProjectInfoFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "ExptProjectInfo";
        ...
        public  ExptProjectInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
