...
namespace EntFrm.Business.BLL
{
    public class MaterialInfoBLL
    {
        ...
        public  MaterialInfoBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(MaterialInfo info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(MaterialInfo  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IMaterialInfo  infoDAL = MaterialInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(MaterialInfo info)
        {
            try
            {
        ...
        public  bool UpdateRecord(MaterialInfo info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  MaterialInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IMaterialInfo  infoDAL = MaterialInfoFactory.Create(this.connStr, this.appCode);
                  MaterialInfoCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  MaterialInfoCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IMaterialInfo  infoDAL = MaterialInfoFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "MaterialInfo";
        ...
        public  MaterialInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
