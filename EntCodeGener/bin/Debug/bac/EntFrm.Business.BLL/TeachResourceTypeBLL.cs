...
{
    public class TeachResourceTypeBLL
    {
        ...
        public  TeachResourceTypeBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(TeachResourceType info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(TeachResourceType  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     ITeachResourceType  infoDAL = TeachResourceTypeFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(TeachResourceType info)
        {
            try
            {
        ...
        public  bool UpdateRecord(TeachResourceType info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  TeachResourceType GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  ITeachResourceType  infoDAL = TeachResourceTypeFactory.Create(this.connStr, this.appCode);
                  TeachResourceTypeCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  TeachResourceTypeCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             ITeachResourceType  infoDAL = TeachResourceTypeFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "TeachResourceType";
        ...
        public  TeachResourceTypeCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
