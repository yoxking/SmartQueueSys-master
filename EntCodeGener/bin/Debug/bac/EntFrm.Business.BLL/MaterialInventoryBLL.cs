...
namespace EntFrm.Business.BLL
{
    public class MaterialInventoryBLL
    {
        ...
        public  MaterialInventoryBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(MaterialInventory info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(MaterialInventory  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IMaterialInventory  infoDAL = MaterialInventoryFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(MaterialInventory info)
        {
            try
            {
        ...
        public  bool UpdateRecord(MaterialInventory info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  MaterialInventory GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IMaterialInventory  infoDAL = MaterialInventoryFactory.Create(this.connStr, this.appCode);
                  MaterialInventoryCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  MaterialInventoryCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IMaterialInventory  infoDAL = MaterialInventoryFactory.Create(this.connStr, this.appCode);
           ...
           s_model.sTableName = "MaterialInventory";
        ...
        public  MaterialInventoryCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
