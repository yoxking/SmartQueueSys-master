...
namespace EntFrm.Business.BLL
{
    public class PlanInfoBLL
    {
        ...
        public  PlanInfoBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(PlanInfo info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(PlanInfo  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IPlanInfo  infoDAL = PlanInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(PlanInfo info)
        {
            try
            {
        ...
        public  PlanInfo GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IPlanInfo  infoDAL = PlanInfoFactory.Create(this.connStr, this.appCode);
                  PlanInfoCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  PlanInfoCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IPlanInfo  infoDAL = PlanInfoFactory.Create(this.connStr, this.appCode);
        ...
        public  PlanInfoCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
