...
namespace EntFrm.Business.BLL
{
    public class PlanTypeBLL
    {
        ...
        public  PlanTypeBLL(string sConnStr, string sAppCode)
        {
           this.connStr = sConnStr;
           this.appCode = sAppCode;
        }

        private  void ValidateRequired(PlanType info)
        {
            /*if (info != null)
            {
        ...
        private  void ValidateRepeat(PlanType  info)
        {
            /*if (info != null)
            {
                 try
                 {
                     IPlanType  infoDAL = PlanTypeFactory.Create(this.connStr, this.appCode);
        ...
        public  bool AddNewRecord(PlanType info)
        {
            try
            {
        ...
        public  bool UpdateRecord(PlanType info)
        {
            try
            {
                 //ValidateRequired(info);
        ...
        public  PlanType GetRecordByNo(string sNo)
        {
            if (sNo.Length > 0)
            {
                try
                {
                  IPlanType  infoDAL = PlanTypeFactory.Create(this.connStr, this.appCode);
                  PlanTypeCollections infoColl = infoDAL.GetRecordsByNo(sNo);
        ...
        public  PlanTypeCollections GetRecordsByPaging(ref int pageCount,int pageIndex,int pageSize,string condition)
        {
          try
          {
             IPlanType  infoDAL = PlanTypeFactory.Create(this.connStr, this.appCode);
        ...
        public  PlanTypeCollections GetRecordsByPaging(SqlModel s_model)
        {
            if (s_model!=null)
            {
                try
