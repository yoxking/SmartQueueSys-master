  ...
  public class ProcessFlowsDAL: IProcessFlows
  {
        ...
        public int AddNewRecord(ProcessFlows info)
        {
            ...
            try
            {
                ...
                paras[5].Value = info.sWFlowsNo;
                paras[6].Value = info.iWFlowsIndex;
                paras[7].Value = info.dEnqueueTime;
                paras[8].Value = info.dBeginTime;
                paras[9].Value = info.dFinishTime;
                paras[10].Value = info.iProcessState;
                paras[11].Value = info.sProcessFormat;
                paras[12].Value = info.iProcessIndex;
                paras[13].Value = info.iPriorityType;
                paras[14].Value = info.iOrderWeight;
                paras[15].Value = info.iPauseState;
                paras[16].Value = info.iDelayType;
                paras[17].Value = info.iDelayTimeValue;
                paras[18].Value = info.iDelayStepValue;
                paras[19].Value = info.dProcessedTime;
                paras[20].Value = info.sProcessedCounterNo;
                paras[21].Value = info.sProcessedStafferNo;
                paras[22].Value = info.sBranchNo;
                paras[23].Value = info.sAddOptor;
                paras[24].Value = info.dAddDate;
                paras[25].Value = info.sModOptor;
                paras[26].Value = info.dModDate;
                paras[27].Value = info.iValidityState;
                paras[28].Value = info.sComments;
                paras[29].Value = info.sAppCode;
        ...
        public int UpdateRecord(ProcessFlows info)
        {
            ...
            try
            {
                ...
                paras[28].Value = info.sComments;
                paras[29].Value = info.sAppCode;
                paras[30].Value = StringHelper.ConvertToBytes(info.sVersion);
