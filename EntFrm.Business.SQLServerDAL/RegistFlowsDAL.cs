...
namespace EntFrm.Business.SQLServerDAL
{
  public class RegistFlowsDAL: IRegistFlows
  {
        ...
        private const string SQL_GET_NAME_BY_NO = @"Select Name From RegistFlows Where   AppCode like @AppCode And   ValidityState=1 And RFlowNo=@RFlowNo";
        private const string SQL_ADD_RECORD = @"Insert into RegistFlows
                                              (RFlowNo,DataFlag,RUserNo,RegistType,DataFrom,RegistDate,ServiceNo,CounterNo,WorkTime,StartDate,EnditDate,RegistState,BranchNo,AddOptor,AddDate,ModOptor,ModDate,ValidityState,Comments,AppCode)
                                              values(@RFlowNo,@DataFlag,@RUserNo,@RegistType,@DataFrom,@RegistDate,@ServiceNo,@CounterNo,@WorkTime,@StartDate,@EnditDate,@RegistState,@BranchNo,@AddOptor,@AddDate,@ModOptor,@ModDate,@ValidityState,@Comments,@AppCode)";
        private const string SQL_UPDATE_RECORD = @"Update RegistFlows set
                                                 RFlowNo=@RFlowNo,DataFlag=@DataFlag,RUserNo=@RUserNo,RegistType=@RegistType,DataFrom=@DataFrom,RegistDate=@RegistDate,ServiceNo=@ServiceNo,CounterNo=@CounterNo,WorkTime=@WorkTime,StartDate=@StartDate,EnditDate=@EnditDate,RegistState=@RegistState,BranchNo=@BranchNo,AddOptor=@AddOptor,AddDate=@AddDate,ModOptor=@ModOptor,ModDate=@ModDate,ValidityState=@ValidityState,Comments=@Comments,AppCode=@AppCode 
        ...
        public int AddNewRecord(RegistFlows info)
        {
            ...
            try
            {
                ...
                paras[3].Value = info.iRegistType;
                paras[4].Value = info.sDataFrom;
                paras[5].Value = info.dRegistDate;
                paras[6].Value = info.sServiceNo;
                paras[7].Value = info.sCounterNo;
                paras[8].Value = info.iWorkTime;
                paras[9].Value = info.dStartDate;
                paras[10].Value = info.dEnditDate;
                paras[11].Value = info.iRegistState;
