...
namespace EntFrm.Business.SQLServerDAL
{
  public class RegistHistoryDAL: IRegistHistory
  {
        ...
        private const string SQL_GET_NAME_BY_NO = @"Select CnName From RegistHistory Where   AppCode like @AppCode And   ValidityState=1 And HFlowNo=@HFlowNo";
        private const string SQL_ADD_RECORD = @"Insert into RegistHistory
                                              (HFlowNo,DataFlag,RUserNo,CnName,Age,Sex,RegistType,DataFrom,RegistDate,ServiceNo,CounterNo,WorkTime,StartDate,EnditDate,RegistState,BranchNo,AddOptor,AddDate,ModOptor,ModDate,ValidityState,Comments,AppCode)
                                              values(@HFlowNo,@DataFlag,@RUserNo,@CnName,@Age,@Sex,@RegistType,@DataFrom,@RegistDate,@ServiceNo,@CounterNo,@WorkTime,@StartDate,@EnditDate,@RegistState,@BranchNo,@AddOptor,@AddDate,@ModOptor,@ModDate,@ValidityState,@Comments,@AppCode)";
        private const string SQL_UPDATE_RECORD = @"Update RegistHistory set
                                                 HFlowNo=@HFlowNo,DataFlag=@DataFlag,RUserNo=@RUserNo,CnName=@CnName,Age=@Age,Sex=@Sex,RegistType=@RegistType,DataFrom=@DataFrom,RegistDate=@RegistDate,ServiceNo=@ServiceNo,CounterNo=@CounterNo,WorkTime=@WorkTime,StartDate=@StartDate,EnditDate=@EnditDate,RegistState=@RegistState,BranchNo=@BranchNo,AddOptor=@AddOptor,AddDate=@AddDate,ModOptor=@ModOptor,ModDate=@ModDate,ValidityState=@ValidityState,Comments=@Comments,AppCode=@AppCode 
        ...
        private const string SQL_GET_RECORDS_BY_CLASSNO = @"Select * From RegistHistory Where    AppCode like @AppCode And   ValidityState=1 And ServiceNo=@ServiceNo";
        ...
        public int AddNewRecord(RegistHistory info)
        {
            ...
            try
            {
                ...
                paras[4].Value = info.iAge;
                paras[5].Value = info.iSex;
                paras[6].Value = info.iRegistType;
                paras[7].Value = info.sDataFrom;
                paras[8].Value = info.dRegistDate;
                paras[9].Value = info.sServiceNo;
                paras[10].Value = info.sCounterNo;
                paras[11].Value = info.iWorkTime;
                paras[12].Value = info.dStartDate;
                paras[13].Value = info.dEnditDate;
                paras[14].Value = info.iRegistState;
