...
{
    public class OrganizInfoController : frmMainController
    {
        ...
        public override ActionResult List()
        {
            try
            {
                ...
                OrganizInfoCollections infoColl = infoBLL.GetRecordsByPaging(ref PageCount, PageIndex, this.PageSize, Condition);
        ...
        //[(Message = "医生科室信息编辑(Edit)")]
        public override ActionResult Edit(string id)
        {
            try
            {
                OrganizInfoBLL infoBLL = new OrganizInfoBLL(PublicHelper.Get_ConnStr(), PublicHelper.Get_AppCode());
                OrganizInfo info = infoBLL.GetRecordByNo(id);
        ...
        public override ActionResult Save()
        {
            ...
            try
            {
                ...
                string OrganizName = Request.Form["OrganizName"].ToString();
                string ParentNo = "00000000";
                int OrderNo = Request.Form["OrderNo"].ToInt();
                string Comments = Request.Form["Comments"].ToString();


                OrganizInfoBLL infoBLL = new OrganizInfoBLL(PublicHelper.Get_ConnStr(), PublicHelper.Get_AppCode());
                OrganizInfo info = infoBLL.GetRecordByNo(OrganizNo);
