...
{
    public class QuartzInfoController : frmMainController
    {
        ...
        public override ActionResult List()
        {
            try
            {
                ...
                DsQuartzInfoBLL infoBLL = new DsQuartzInfoBLL(PublicHelper.Get_ConnStr(), PublicHelper.Get_AppCode());
                DsQuartzInfoCollections infoColl = infoBLL.GetRecordsByPaging(ref PageCount, PageIndex, this.PageSize, Condition);
        ...
        public override ActionResult Add()
        {
            try
            {
                DsQuartzInfo info = new DsQuartzInfo();
                info.sQuartzNo = CommonHelper.Get_New12ByteGuid();
                info.sJobTask = "Shutdown";
                info.iWeekDay = 1;
                info.sPlayerNos = " "; 
        ...
        public override ActionResult Edit(string id)
        {
            ...
            {
                DsQuartzInfoBLL infoBLL = new DsQuartzInfoBLL(PublicHelper.Get_ConnStr(), PublicHelper.Get_AppCode());
                DsQuartzInfo info = infoBLL.GetRecordByNo(id);
        ...
        private List<ItemObject> getPlayerList()
        {
            List<ItemObject> plist = new List<ItemObject>();

            int count = 0;
            DsPlayerInfoBLL infoBLL = new DsPlayerInfoBLL(PublicHelper.Get_ConnStr(), PublicHelper.Get_AppCode());
            DsPlayerInfoCollections infoColl = infoBLL.GetRecordsByPaging(ref count, 1, 100, "CheckState=1");

            if (infoColl != null && infoColl.Count > 0)
            {
                foreach (DsPlayerInfo info in infoColl)
                {
                    plist.Add(new ItemObject(info.sPlayerCode, info.sPlayerName));
                }
            }
        ...
        public override ActionResult Save()
        {
            ...
            try
            {
                ...
                string sNo = Request.Form["QuartzNo"].ToString();
                string QuartzName = Request.Form["QuartzName"].ToString();
                string CornExp = Request.Form["CornExp"].ToString();
                string JobTask = Request.Form["JobTask"].ToString();
                string WeekDay = Request.Form["WeekDay"].ToString();
                string PlayerCodes = Request.Form["PlayerCodes"].ToString(); 
                string Comments = Request.Form["Comments"].ToString();

                DsQuartzInfoBLL infoBLL = new DsQuartzInfoBLL(PublicHelper.Get_ConnStr(), PublicHelper.Get_AppCode());
                DsQuartzInfo info = infoBLL.GetRecordByNo(sNo);
                ...
                if (info == null)
                {
                    ...
                    info.sQuartzNo = sNo;
                    info.sQuartzName = QuartzName;
                    info.sCornExp = CornExp;
                    info.sJobTask = JobTask;
                    info.iWeekDay = int.Parse(WeekDay);
                    info.sPlayerNos = PlayerCodes;
                    info.sSummary ="";
